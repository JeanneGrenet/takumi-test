...
interface PostOGImageParams {
  params: { id?: string; __image?: string };
  site?: URL;
  url: URL;
}
...
export default async function PostOGImage({ params, url }: PostOGImageParams) {
  const postId = url.searchParams.get("id") || params.id;
  ...
  const { title, description, tags = [], author, date } = post.data;
